...
let config = {
  ICOs: {
    ...
    '0xD566Fa4a696EAc66f749f7fe999D6673fEe2026c': {
      information: {
        aliasName: 'CHEX',
      ...
      events: {
        Transfer: {
          ...
          firstTransactionBlockNumber: 4204457,
          lastTransactionBlockNumber: 4550057,
          countTransactions: true,
        }
      },
      icoParameters: {
        ...
        endDate: async icoContract => 'not provided',
        status: async (web3, icoContract) => {
          const frozen = await toPromise(frozen)().valueOf();
          return frozen ? 'not provided' : 'in progress';
        },
      },
      matrix: {
        q1: { answer: true },
        ...
        q6: { answer: true },
        q7: { answer: true },
        q8: { answer: true, comment: 'Adjusts to estimate current bitcoin:ethereum prices, and generates a receipt for real time transaction' },
        q9: { answer: false, comment: 'Non-ethereum transactions (bitcoin, altcoins) are processed primarily through a centralized server' },
        q10: { answer: true, comment: 'Simplified and uses common practices' },
        q11: { answer: true, comment: 'The only exception to this is that the contract uses block time to change phases, which can be lead to slightly early/late dates depending on network congestion' },
