...
const CustomTooltip = (e) => {
  ...
  if (e.payload && e.payload.length >= 1 && e.payload[0].payload.tooltipName)
    key = e.payload[0].payload.tooltipName;

  return (
    <div className="custom-tooltip">
      <strong>{e.yLabel}</strong><span> {formatNumber(value, 2)}</span>
      <br />
      <strong>{e.xLabel}</strong><span> {key}</span>
    </div>);
}
export default ({ title, data, dataKey, xLabel, yLabel, isVisible = true, isNotVisibleMessage = '', hideTitle, tooltip = {} }) => {
  const { xTitle, yTitle } = tooltip;
  return (
