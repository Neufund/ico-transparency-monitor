...
In addition a number of generated graphs from these ICOs help assess statistically these ICOs, These graphs can be direcly taken and used for publication and articles related to ICO analsys.
...
## How it works
Every transaction that occurs in the network is loged in the public etheruem leger. Smart-Contracts when executed generate diffrent events based on how they are programed and the action they execute. Each contract generates these [log](http://solidity.readthedocs.io/en/develop/introduction-to-smart-contracts.html#logs) events with the smart-contracts Ether address as refrence, Using [Web3](https://github.com/ethereum/wiki/wiki/JavaScript-API#watch-callback-return-value) The ICO-Monitor takes each contract in it's contract and scans through the blocks returning token creation events generated by this smart-contract.  
